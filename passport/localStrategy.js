...
module.exports = () => {
    passport.use(new LocalStrategy({ // LocalStrategy 생성자의 첫 번째 인수(객체): 전략에 관한 설정
        usernameField: 'localId', // body-parser에 의해 req.body 내의 속성명(req.body.email)
        passwordField: 'password',
    }, async (localId, password, done) => { // LocalStrategy 생성자의 두 번째 인수(함수): 실제 전략 수행
                                         // 첫 번째 인수에서 넣어주었던 email, password가 여기의 매개변수가 됨
                                        // done은 routes/auth.js의 passport.authenticate의 콜백 함수
        try {
            // console.log(11111111);
            // console.log(await User.findOne({where: { localId }}));
            // console.log(11111111);
            const exUser = await User.findOne({ where: { localId }}); // db에 일치하는 Email이 있는지 확인
