...
router.post('/follow/:id', isLoggedIn, async (req, res, next) => {
    try {
        ...
        const finduser = await User.findOne({ where: { localId: req.params.id}});
        if(user && finduser) {
            await user.addFollowing(finduser.id); // :id 부분: req.params.id임( : = params, id = id), sequelize에서 추가한 addFollowing 메서드로 현재 로그인한 사용자와의 관계 지정, 10: 10진수
